...
int main() {
  ...
  double energyDrinkBuyers = TOTAL_CUSTOMERS * (ENERGY_DRINK_PURCHASE_PCT / 100.0);
  //calculate number who prefer citrus flavor
  double citrusPreferers = energyDrinkBuyers * (CITRUS_PREFERENCE_PCT / 100.0);
  //round to nearest whole number for realistic reporting
  int energyDrinkBuyersRounded = static_cast<int>(energyDrinkBuyers + 0.5);
  int citrusPreferersRounded = static_cast<int>(citrusPreferers + 0.5);
  ...
  cout << left << setw(45)
    ...
    << right << setw(6) << energyDrinkBuyersRounded << " (" << fixed << setprecision(1) << ENERGY_DRINK_PURCHASE_PCT << "%)" << endl;
  cout << left << setw(45)
